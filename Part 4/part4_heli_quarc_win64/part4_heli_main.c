...
typedef RT_MODEL_part4_heli_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * part4_heli(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(part4_heli_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(part4_heli_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://part4_heli:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: part4_heli -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(part4_heli_DW.HILReadEncoderTimebase_Task,
        (t_clock) part4_heli_P.HILReadEncoderTimebase_clock, 500.0, -1)
        ;
      if (result == 0) {
